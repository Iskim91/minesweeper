...
const grabAdjacentCells = (cells: Cell[][], rowParam: number, colParam: number): {
...
export const generateCells = (): Cell[][] => {
    // its an array of array that contains value and state
    let cells: Cell[][] = [];
    ...
    for (let rowIndex = 0; rowIndex < MAX_ROWS; rowIndex++) {
        for (let colIndex = 0; colIndex < MAX_COLS; colIndex++){
            ...
            if (topLeftCell?.value === CellValue.bomb) {
                // same as above(topLeftBomb && topLeftBomb.value === CellValue.bomb) {
                numberOfBombs++;
            }
